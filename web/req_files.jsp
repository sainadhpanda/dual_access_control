                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="section-heading">
                            <center><h2>Requested Files</h2></center><br>
                        </div>
                        <table id="customers">
                            <tr>
                                <th>File ID</th>
                            ...
                            <%
                                ...
                                try {
                                    ResultSet rs = st.executeQuery("SELECT * FROM request WHERE uid = '"+ duid +"'");
                                    while (rs.next()) {
                            %>
                            <tr>
                                <td><%=rs.getString("fid")%></td>
                                <td><%=rs.getString("filename")%></td>
                                <td><%=rs.getString("authority_status")%></td>
                                <td><%=rs.getString("enclave_status")%></td>
                                <td><%=rs.getString("time")%></td>
                                <td><a href="view_requested.jsp?fid=<%=rs.getString("fid")%>&time=<%=rs.getString("time")%>" class="btn btn-success">Download</a></td>
                            </tr>
                            <%                                        }
                                } catch (Exception ex) {
                                    ex.printStackTrace();
